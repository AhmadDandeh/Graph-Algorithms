...
public class GhaithAlgorithm {
    ...
    public GhaithAlgorithm(int[][] a, int[] b) {
        this.n = a.length;
        this.graphArray1 = (int[][])Arrays.copyOf(a, this.n);
        this.r = (double)(1000 / this.n);
        this.graphArray = new int[this.n][this.n];
    ...
    private void CalcNewItemNumber() {
        ...
        for(int i = 0; i < this.newArray.size() - 1; ++i) {
            for(int j = i + 1; j < this.newArray.size(); ++j) {
                if ((Integer)this.newArray.get(i) > (Integer)this.newArray.get(j)) {
                    int t = (Integer)this.newArray.get(i);
                    this.newArray.add(i, (Integer)this.newArray.get(j));
                    this.newArray.remove(i + 1);
                    this.newArray.add(j, t);
                    this.newArray.remove(j + 1);
                }
            }
        }
